...
void    set_resolution(t_parameters *params, const char *line)
{
	...
	if (params->win_w != 0 || params->win_h != 0) 
		quit_program(params, "Error: resolution has already been set.");
	params->win_w = ft_atoi(line);
	if (params->win_w > MAX_WINDOW_WIDTH) 
		params->win_w = MAX_WINDOW_WIDTH;
	if (params->win_w < 1)
		quit_program(params, "Error: can't run a window with given width.");
	i = 0;
	while (line[i] == 32)
		i++;
	...
	if (params->win_h < 1)
		quit_program(params, "Error: can't run a window with given height.");
}
